...
public class Consumer2 {
    ...
    public void consumer() throws IOException, TimeoutException {
        ...
        String msg = channel.basicConsume(RabbitConstant.TOPIC_QUEUES_2, false, new DefaultConsumer(channel){
            @Override
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {

                String message = new String(body);
                System.out.println(envelope.getRoutingKey()+" 消费者接收到的消息："+message);

                System.out.println(envelope.getRoutingKey()+" 消息的TagId："+envelope.getDeliveryTag());
                //手动提交 ack
